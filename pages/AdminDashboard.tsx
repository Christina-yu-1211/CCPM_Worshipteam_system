...
import { Users, Bus, Utensils, Calendar, PlusCircle, Trash2, Download, UserPlus, CheckSquare, FileText, ChevronDown, ChevronUp, BarChart2, TrendingUp, X, Edit, AlertTriangle, Palette, CheckCircle, Archive, Key, Clock } from 'lucide-react';
...
interface AdminDashboardProps {
  ...
  onUpdateSignup?: (id: string, data: Partial<Signup>) => Promise<void>;
}

export const AdminDashboard: React.FC<AdminDashboardProps> = ({
  currentUser, events, signups, series, users, tasks,
  onAddEvent, onUpdateEvent, onDeleteEvent, onUpdateTask, onAddTask, onDeleteTask, onManageUser, onSeriesAction, onTriggerEmailCheck, onUpdateSignup
}) => {
  ...
  const handleEarlyArrivalAction = async (signup: Signup, status: 'APPROVED' | 'REJECTED') => {
    const event = events.find(e => e.id === signup.eventId);
    if (!event) return;

    const updateData: Partial<Signup> = { earlyArrivalStatus: status };

    if (status === 'REJECTED') {
      updateData.arrivalDate = event.startDate;
      const firstDay = event.startDate;
      if (signup.attendingDays.length > 0) {
        updateData.attendingDays = signup.attendingDays.filter(d => d >= firstDay);
      }
    } else {
      // APPROVED
      if (signup.arrivalDate && !signup.attendingDays.includes(signup.arrivalDate)) {
        updateData.attendingDays = [signup.arrivalDate, ...signup.attendingDays].sort();
      }
    }

    if (onUpdateSignup) {
      await onUpdateSignup(signup.id, updateData);
    }
  };
          ...
          {displayedEvents.map(evt => {
            ...
            return (
              <div key={evt.id} className="bg-white rounded-3xl shadow-sm border border-gray-200 overflow-hidden transition-all duration-300">
                ...
                {/* Expanded Content */}
                {isExpanded && (
                  <div className="border-t border-gray-100 bg-gray-50/50 p-4 md:p-8 space-y-8 animate-fade-in">

                    {/* 0. Early Arrival Requests (If any) */}
                    {evtSignups.filter(s => s.earlyArrivalStatus === 'PENDING').length > 0 && (
                      <div className="bg-amber-50 rounded-2xl border-2 border-amber-200 p-6 space-y-4 shadow-sm">
                        <h4 className="text-lg font-black text-amber-900 flex items-center gap-2">
                          <Clock size={20} className="text-amber-600" /> 提早上山申請待處理 ({evtSignups.filter(s => s.earlyArrivalStatus === 'PENDING').length})
                        </h4>
                        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                          {evtSignups.filter(s => s.earlyArrivalStatus === 'PENDING').map(req => {
                            const vol = volunteerMap[req.volunteerId];
                            return (
                              <div key={req.id} className="bg-white rounded-xl border border-amber-100 p-4 shadow-sm flex flex-col justify-between">
                                <div>
                                  <div className="flex justify-between items-start mb-2">
                                    <span className="font-extrabold text-gray-800 text-lg">{vol?.name}</span>
                                    <span className="text-xs font-black bg-amber-100 text-amber-700 px-2 py-1 rounded">
                                      {req.earlyArrivalType === '1_day' ? '提早 1 天' : req.earlyArrivalType === '2_days' ? '提早 2 天' : '提早'}
                                    </span>
                                  </div>
                                  <p className="text-sm text-gray-500 mb-4 bg-gray-50 p-2 rounded italic font-medium">「{req.earlyArrivalReason || '無備註'}」</p>
                                </div>
                                <div className="flex gap-2">
                                  <button
                                    onClick={() => handleEarlyArrivalAction(req, 'APPROVED')}
                                    className="flex-1 py-2 bg-green-500 hover:bg-green-600 text-white font-bold rounded-lg text-sm transition active:scale-95"
                                  >核准</button>
                                  <button
                                    onClick={() => handleEarlyArrivalAction(req, 'REJECTED')}
                                    className="flex-1 py-2 bg-red-400 hover:bg-red-500 text-white font-bold rounded-lg text-sm transition active:scale-95"
                                  >拒絕</button>
                                </div>
                              </div>
                            );
